...
class User(db.Model):
  ...
  def add_todo_category(self, todo_id, category):
    try:
      ...
      if not todo:
          return False
      category_instance = Category(self.id, category)
      db.session.add(category_instance)
      db.session.commit()
      todo_category_instance = TodoCategory(todo_id=todo.id, category_id=category_instance.id)
      db.session.add(todo_category_instance)
      db.session.commit()
      return True
  ...
  #   todo = Todo.query.filter_by(id=todo_id, user_id=self.id).first()
  #   if todo:
  #     category_rec = Category.query.filter_by(      )
  #     if not category_rec:
